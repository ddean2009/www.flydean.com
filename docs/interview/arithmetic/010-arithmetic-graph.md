...
构建图：就是构建每个节点和其他节点之间的关系。可以用edges = new ArrayList<List<Integer>>()这个**二维list**来表示。
...
解法：广度优先搜索：将节点编号和到达该节点的移动次数作为搜索状态，顺着该节点的出边扩展新状态，直至到达终点 N^2
...
## [127. 单词接龙--同443最小基因突变](https://leetcode.cn/problems/word-ladder/)
...
看到最短首先想到的就是**广度优先搜索**。想到**广度优先搜索**自然而然的就能想到图。
